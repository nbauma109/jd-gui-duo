...
public class LocalVariableAnalyzer
{
			...
			List<Instruction> list, List<Instruction> listForAnalyze)
	{
		...
		else
		{
			...
			{
				// Dans le cas des m�thodes g�n�r�es automatiquement par le
				...
				if (astore.getValueref().getOpcode() == ByteCodeConstants.EXCEPTIONLOAD)
				{
					...
					if (el.getExceptionNameIndex() != 0)
					{
						...
						if (lv == null)
						{
							...
							{
								// Variable trouv�e. Mise � jour de 'startPc' de la
								// port�e.
			...
			byte[] code, LocalVariables localVariables, IndexInstruction ii)
	{
		...
		if (lv == null)
		{
			...
			else
			{
				...
				// variable possedant le m�me index et precedement
				// definie.
	...
	//   }
	/*
	 * Fusion des entrees du tableau poss�dant les m�mes num�ro de slot,
	 * le m�me nom et le m�me type. Le tableau genere pour le code suivant
			...
			List<Instruction> listForAnalyze, String returnedSignature)
	{
		...
		for (int i=0; i<length; i++)
		{
			...
			case OBJECT_TYPE:
				...
				// �tre ajout�s. Voir la limitation de JAD sur ce point.
			...
			Instruction instruction)
	{
		...
		{
			// variable non trouv�e ou type incompatible => cr�ation de variable
			localVariables.add(new LocalVariable(
			...
			List<Instruction> listForAnalyze, LocalVariables localVariables)
	{
		 ...
		 * initialis�es. Cela se produit lorsque les m�thodes poss�dent un bloc

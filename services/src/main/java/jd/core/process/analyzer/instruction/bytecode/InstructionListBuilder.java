			...
			List<Instruction> listForAnalyze)
	{
		...
		if (code != null)
		{
			...
			try
			{
				...
				// adresse de retour doit �tre inseree sur la pile.
				IntSet offsetSet = new IntSet();

				// Population des deux tableaux dans la m�me passe.
				...
				for (offset=0; offset<length; ++offset)
				{
					...
					if (offset == exceptionOffset && codeExceptions != null)
					{
						...
						// d'exception en d�but de bloc catch
						int catchType =
					...
					{
						// Ajout d'une pseudo adresse de retour en d�but de
					...
					// Traitement des num�ros de ligne

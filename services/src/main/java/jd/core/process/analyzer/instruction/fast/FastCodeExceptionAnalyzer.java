...
public class FastCodeExceptionAnalyzer
{
			...
			Method method, List<Instruction> list)
	{
		...
		// Aggregation des 'finally' et des 'catch' executant le m�me bloc
			...
			List<Instruction> list, FastAggregatedCodeExcepcion face)
	{
		 ...
		 * monitorexit ayant le m�me index que l'instruction
		...
		// Si l'expression ci-dessous est vraie, aucune instruction 'MonitorExit' n'a ete trouv�e. Cas de la
			...
			List<Instruction> list, FastAggregatedCodeExcepcion fastCodeException)
	{
		...
		// Approximation a affin�e par la m�thode 'ComputeAfterOffset'
		fce.setAfterOffset(searchAfterOffset(list, fastCodeException.getHandlerPC()));

		return fce;
	}

	/** Recherche l'offset apr�s le bloc try-catch-finally. */
	private static int searchAfterOffset(List<Instruction> list, int offset)
	{
		...
		case Const.RET:
			...
			// Strategie : recheche de l'instruction goto, sautant apr�s
			...
			FastCodeExcepcion fastCodeException, int fastCodeExceptionIndex)
	{
		...
		case FastConstants.TYPE_JIKES_122:
			// Le traitement suivant etait faux pour reconstruire la m�thode
		...
		case FastConstants.TYPE_ECLIPSE_677_FINALLY:
		{
			...
			if (instruction.getOpcode() == Const.POP) {
				...
				// - aucun 'goto' ne saute apr�s le dernier bloc finally.
			...
			if (index < list.size()) {
						...
						list.get(index).getOpcode() == Const.GOTO)
				{
							...
							jumpOffset <= list.get(indexTmp).getOffset())
					{
						...
						// Reduction de 'afterOffset' a l'aide des num�ros de ligne
		...
		case FastConstants.TYPE_118_FINALLY:
		{
			...
			// Strategie : le bon offset, apr�s le bloc 'try-finally', se
			// trouve apr�s l'instruction 'ret' de la sous procedure du
			// bloc 'finally'.
			int index = InstructionUtil.getIndexForOffset(
					list, fastCodeException.getFinallyFromOffset());
			if (index <= 0) {
		...
		default:
		{
			...
			// Recherche de la 1ere exception d�butant apr�s 'maxOffset'
				...
				else
				{
					...
					case Const.ATHROW,
						...
						} else if (index+1 >= length)
						{
							...
							{
								// Dernier 'return'
								// => Instruction placee apr�s le bloc
							...
							if (jumpOffsetTmp >= afterOffset) {
										...
										jumpOffsetTmp != afterOffset)
								{
									...
									else
									{
										...
										// 'try-catch-finally' saute vers
										// cette instuction
										// => Instruction plac�e apr�s le bloc
										fastCodeException.setAfterOffset(instruction.getOffset());
									}
								}
								//else
								...
								// et si l'offset de saut est le m�me que 'afterOffset',
			...
			if (firstLineNumber != Instruction.UNKNOWN_LINE_NUMBER)
			{
				...
				if (index != -1)
				{
					while (index-- > 0)
					{
						...
						// L'instruction a un num�ro de ligne inferieur aux
						// instructions du bloc 'try'. A priori, elle doit �tre
						// place apr�s le bloc 'catch'.

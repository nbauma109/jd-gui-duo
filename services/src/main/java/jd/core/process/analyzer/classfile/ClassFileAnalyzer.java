...
public class ClassFileAnalyzer
{
			...
			ClassFile classFile)
	{
		...
		else
		{
			...
			// externe. 'PreAnalyzeMethods' doit �tre execute avant l'analyse
			...
			try
			{
				...
				else
				{
							...
							(Const.ACC_SYNTHETIC|Const.ACC_BRIDGE)) == 0)
					{
						...
						// Recherche du num�ro de l'attribut contenant la reference
			...
			ClassFile classFile)
	{
		...
		// m�thodes externes si la classe courante est une classe interne ou
		...
		for (int i=0; i<length; i++)
		{
			...
			try
			{
				...
				// Cette operation doit �tre executee avant
				...
				// Cette operation doit �tre executee apr�s 'DupStoreThisReconstructor'.

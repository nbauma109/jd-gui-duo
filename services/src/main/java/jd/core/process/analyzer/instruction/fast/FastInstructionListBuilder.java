...
public class FastInstructionListBuilder {
    ...
    /**
     * d�but de liste fin de liste | | Liste ...
     * --|----|---|==0===1===2===3===4===5===6==7=...=n---|--| ...
     */
    public static void build(ReferenceMap referenceMap, ClassFile classFile, Method method, List<Instruction> list) {
        ...
        // Agregation des d�clarations CodeException
        ...
        if (fce.getType() == FastConstants.TYPE_118_FINALLY) {
            ...
            if (list.get(--index).getOpcode() == Const.MONITORENTER) {
                ...
                if (index + 1 < list.size() && list.get(index + 1).getOpcode() == ByteCodeConstants.XRETURN) {
                    // Si l'instruction retourn�e poss�de un offset inf�rieur �
    ...
    private static void executeReconstructors(ReferenceMap referenceMap, ClassFile classFile, List<Instruction> list,
        ...
        // A executer apr�s 'AssignmentInstructionReconstructor',
        // 'IfGotoToIfReconstructor'
        // A executer avant 'TernaryOpReconstructor'
        ComparisonInstructionAnalyzer.aggregate(list);
        // Recontruction des instructions 'assert'. Cette operation doit �tre
        // executee apr�s 'ComparisonInstructionAnalyzer'.
        ...
        // Cette operation doit �tre executee apr�s
    ...
    /**
     * Effacement de instruction 'return' inutile sauf celle en fin de m�thode
     ...
     *
     *
     * beforeLoopEntryOffset & loopEntryOffset: utile pour la g�n�ration
     * d'instructions 'continue' beforeListOffset: utile pour la g�n�ration de
     * d�clarations de variable endLoopOffset & afterLoopOffset: utile pour la
     * g�n�ration d'instructions 'break' afterListOffset: utile pour la
     * g�n�ration d'instructions 'if-else' = lastBodyWhileLoop.offset
     *
     * WHILE instruction avant boucle | goto | beforeSubListOffset instructions
     * | instruction | beforeLoopEntryOffset if � saut n�gatif |
     * loopEntryOffset, endLoopOffset, afterListOffset instruction apr�s boucle
     * | afterLoopOffset
     *
     * DO_WHILE instruction avant boucle | beforeListOffset instructions |
     * instruction | beforeLoopEntryOffset if � saut n�gatif | loopEntryOffset,
     * endLoopOffset, afterListOffset instruction apr�s boucle | afterLoopOffset
     *
     * FOR instruction avant boucle | goto | beforeListOffset instructions |
     * instruction | beforeLoopEntryOffset iinc | loopEntryOffset,
     * afterListOffset if � saut n�gatif | endLoopOffset instruction apr�s
     * boucle | afterLoopOffset
     *
     *
     * INFINITE_LOOP instruction avant boucle | beforeListOffset instructions |
     * instruction | beforeLoopEntryOffset goto � saut n�gatif |
     * loopEntryOffset, endLoopOffset, afterListOffset instruction apr�s boucle
     * | afterLoopOffset
     */
            ...
            int afterBodyLoopOffset, int beforeListOffset, int afterListOffset, int breakOffset, int returnOffset)
    {
        ...
        // Recontruction de la sequence 'return (b1 == 1);' apr�s la
        ...
        createContinue(list, beforeLoopEntryOffset, loopEntryOffset, returnOffset);
        ...
        // // A executer avant l'ajout des d�clarations.
        // StoreReturnAnalyzer.Cleanup(list, localVariables);

        // Add local variable d�clarations
     ...
     * Si elles referencent une variables locales non encore d�clar�e et dont la
     ...
     * non encore d�clar�es et dont la port�e est incluse � la liste courante,
     * on declare les variables en d�but de bloc.
     */
    ...
    private static void createBreakAndContinue(Method method, List<Instruction> list, IntSet offsetLabelSet,
        ...
        for (int index = 0; index < length; index++)
        {
            ...
            if (ByteCodeUtil.isIfInstruction(instruction.getOpcode(), true))
            {
                ...
                } else // Si la m�thode retourne 'void' et si l'instruction
                ...
                if (ByteCodeUtil.jumpTo(method.getCode(), jumpOffset, returnOffset)) {
                    ...
                    // duplication de l'instruction cible pour �viter la
                    // g�n�ration d'une instruction *_LABELED_BREAK.
                    byte[] code = method.getCode();

                    // Reconnaissance bas niveau de la sequence
                    // '?load_?' suivie de '?return' en fin de m�thode.
            ...
            else if (instruction.getOpcode() == Const.GOTO)
            {
                ...
                if (beforeLoopEntryOffset < jumpOffset && jumpOffset <= loopEntryOffset) {
                    // L'instruction 'goto' saute vers le d�but de la boucle
                    ...
                    if (code.length == jumpOffset+2)
                    {
                        ...
                        if (load != null)
                        {
                            ...
                            if (ri != null)
                            {
                                ...
                                // '?store' sur la m�me variable et si
                                // elle a le m�me num�ro de ligne
                                // => aggregation
                                if (index > 0)
                                {
    ...
    private static int unoptimizeIfElseInLoop(ClassFile classFile, Method method, List<Instruction> list,
        ...
        if (firstLoopInstructionIndex != -1) {
            ...
            if (index + 1 < length) {
                ...
                // l'instruction vers laquelle le saut n�gatif pointe.
                ...
                if (afterLoopJumpOffset > afterLoopInstructionOffset) {
                    ...
                    if (afterLoopInstructionIndex != -1) {
                        ...
                        // Check next instructions
                            ...
                            // Attention: le goto genere a le m�me offset que
    ...
    /**
     * d�but de liste fin de liste | testIndex | | | | Liste ...
     * --|----|---|==0===1===2===3===4===5===6==7=...=n---|--| ... | | | | | | |
     * beforeListOffset | | Offsets | loopEntryOffset endLoopOffset |
     * beforeLoopEntryOffset afterLoopOffset
     *
            ...
            if (subListLength > 0) {
                ...
                if (subListLength > 1) {
                    ...
                    if (!InstructionUtil.checkNoJumpToInterval(subList, 0, subListLength, lastBodyLoop.getOffset(),
                            test.getOffset())) {
                        // 'lastBodyLoop' ne peut pas �tre l'instruction
    ...
    /**
     * d�but de liste fin de liste | gotoIndex | | | | Liste ...
     * --|----|---|==0===1===2===3===4===5===6==7=...=n---|--| ... | | | | | | |
     * beforeListOffset | | Offsets | loopEntryOffset endLoopOffset |
     * beforeLoopEntryOffset afterLoopOffset
     */
    ...
    private static void analyzeIfAndIfElse(ClassFile classFile, Method method, List<Instruction> list,
        ...
        {
            // L'instruction saute sur un d�but de boucle et la liste termine
        ...
        if (index < length) {
            ...
            if (minusJumpOffset == -1
                ...
                // num�ro de ligne est inf�rieur � l'instruction precedente,
            ...
            if (minusJumpOffset != -1)
            {
                    ...
                    minusJumpOffset <= loopEntryOffset)
                {
                        ...
                        afterBodyLoopOffset == afterListOffset)
                    {
                        ...
                        // participent tout de m�me � une instruction if-else
                        // L'instruction saute sur un d�but de boucle et la liste
    ...
    /**
     * d�but de liste fin de liste | switchIndex | | | | Liste ...
     * --|----|---|==0===1===2===3===4===5===6==7=...=n---|--| ... | | | | | | |
     * beforeListOffset | | Offsets | loopEntryOffset endLoopOffset |
     * beforeLoopEntryOffset afterLoopOffset
     */

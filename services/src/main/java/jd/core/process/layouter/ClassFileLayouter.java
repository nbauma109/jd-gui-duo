...
//            		spw, referenceMap, classFile, field.getAttributes());
    ...
    /**
     * @return liste de sequences de 'LayoutBlock'
     * Sequence produite pour chaque m�thode:
...
//						spw, referenceMap, classFile, method.getAttributes());
            ...
            // apr�s les m�thodes sans code. Evite d'instancier un object
    ...
    {
        // Detection de l'ordre de g�n�ration des champs par le compilateur:
        ...
        if (order == 2) {
            ...
            // Tri par ordre croissant, les blocs sans num�ro de ligne
     ...
     * 3 jeux de cartes,
     * Conserver l'ordre naturel jusqu'� une impossibilite:
     * Copie des blocs sans num�ro de ligne des champs au plus tot
     * Copie des blocs sans num�ro de ligne des m�thodes et des classes internes au plus tard
     */
        ...
        List<SubListLayoutBlock> sortedInnerClassBlockList)
    {
        ...
        // Recherche du bloc ayant un num�ro de ligne defini
        ...
        while (!sortedFieldBlockList.isEmpty())
        {
            if (minLineNumberMethod == Instruction.UNKNOWN_LINE_NUMBER)
            {
                ...
                // Copie des champs avec et sans num�ro de ligne
                maxLineNumber = exclusiveMergeFieldBlockList(
                    layoutBlockList, sortedFieldBlockList,
                    minLineNumberInnerClass, maxLineNumber);
                // Copie de toutes les m�thodes sans num�ro de ligne
                maxLineNumber = mergeBlockList(
                    layoutBlockList, sortedMethodBlockList, maxLineNumber);
                // Copie des classes internes jusqu'� l'inner classe ayant
                // le plus petit num�ro de ligne
                ...
                minLineNumberMethod < minLineNumberInnerClass)
            {
                ...
                // Copie des m�thodes jusqu'� la m�thode ayant le plus
                // petit num�ro de ligne
                maxLineNumber = inclusiveMergeBlockList(
            ...
            else
            {
                ...
                maxLineNumber = exclusiveMergeFieldBlockList(
                    layoutBlockList, sortedFieldBlockList,
                    minLineNumberInnerClass, maxLineNumber);
                // Copie des m�thodes avec et sans num�ro de ligne
        ...
        while (!sortedMethodBlockList.isEmpty())
        {
            ...
            maxLineNumber = exclusiveMergeMethodOrInnerClassBlockList(
                layoutBlockList, sortedMethodBlockList,
                minLineNumberInnerClass, maxLineNumber);
            // Copie des classes internes jusqu'� l'inner classe ayant le
            // plus petit num�ro de ligne
            maxLineNumber = inclusiveMergeBlockList(
                layoutBlockList, sortedInnerClassBlockList,
                minLineNumberInnerClass, maxLineNumber);
            minLineNumberInnerClass =
...
//			case LayoutBlockConstants.SEPARATOR_BEFORE_OR_AFTER_BLOCK,
        ...
        int firstIndex, int lastIndex, int delta)
    {
        ...
        do
        {
            ...
            LayoutBlock lb;
            // Compact d�but de bloc des m�thodes
            for (int i=lastIndex; i>=firstIndex && delta>0; i--)
            {
                lb = layoutBlockList.get(i);
        ...
        do
        {
            ...
            LayoutBlock lb;
            // Compact fin de bloc des m�thodes
            for (int i=lastIndex; i>=firstIndex && delta>0; i--)
            {
                lb = layoutBlockList.get(i);
...
//		while ((delta>0) && (oldDelta>delta));
        ...
        int firstIndex, int lastIndex, int delta)
    {
        ...
        do
        {
            ...
            // Expand fin de bloc des m�thodes
            for (int i=firstIndex; i<=lastIndex && delta>0; i++)
            {
                LayoutBlock lb = layoutBlockList.get(i);
            ...
            LayoutBlock lb;
            // Expand d�but de bloc du corps des classes internes
            for (int i=firstIndex; i<=lastIndex && delta>0; i++)
            {
                lb = layoutBlockList.get(i);
        ...
        LayoutSection lsSource, int markerStartTag, int markerEndTag)
    {
        ...
        while (firstBlockIndex < blockIndex)
        {
            ...
            if (lb.getTag() == markerEndTag)
            {
                ...
                {
                    // Le marqueur de d�but est avant la limite.
                    return false;
                }
                ...
                //  Trouver le premier block de type 'tag' sans num�ro de ligne
                int counter = 1;
                ...
                //  Trouver la section ayant le score le plus bas jusqu'� la
                //  section contenant un block de type 'tag' ayant un num�ro
                //  de ligne defini
                int blockLength = layoutBlockList.size();
...
//						(lb.tag == LayoutBlockConstants.SEPARATOR_BEFORE_OR_AFTER_BLOCK) ||
                    ...
                    {
                        // Fin de corps ou d�but d'un bloc
                ...
                if (lowerScore != lsSource.getScore())
                {
                    ...
                    // Rechercher de l'index de d�but du bloc de type 'tag'
        ...
        LayoutSection lsSource, int markerStartTag, int markerEndTag)
    {
        ...
        while (blockIndex < lastBlockIndex)
        {
            ...
            if (lb.getTag() == markerStartTag)
            {
                ...
                // Rechercher du nombre de blocs a deplacer =>
                //  Trouver le dernier block de type 'tag' sans num�ro de ligne
                int counter = 1;
...
//						(lb.tag == LayoutBlockConstants.SEPARATOR_BEFORE_OR_AFTER_BLOCK) ||
                    ...
                    {
                        // d�but de corps ou fin d'un bloc
                        if (lowerScore > lb.getSection().getScore())
                        {
                            lowerScore = lb.getSection().getScore();
                            lowerScoreBlockIndex = blockIndex;
